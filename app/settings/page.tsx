...
import { AppControlSettings, HolidaySettings, getAppControlSettings, getClosureDaysCount, getHolidaySettings, setAppControlSettings, setHolidaySettings } from "@/lib/utils";
...
function HeadSettings() {
  ...
  useEffect(() => {
    ...
    apiClient.get("/site-settings/app-controls").then((data: any) => {
      if (data?.settings) {
        const merged = { ...getAppControlSettings(), ...data.settings };
        setAppControl(merged);
        setAppControlSettings(merged);
      }
    }).catch(() => undefined);
    apiClient.get("/institution/sms-settings").then((data: any) => {
  ...
  const saveColors = () => runSave("colors", async () => { setAppControlSettings(appControl); await apiClient.put("/site-settings/app-controls", appControl); }, "রঙসমূহ সংরক্ষণ করা হয়েছে।");
  const saveSmsSettings = () => runSave("sms", async () => { const payload: any = { smsEnabled }; if (smsKey.trim()) payload.smsApiKey = smsKey.trim(); await apiClient.post("/institution/sms-settings", payload); setSmsKeySet(true); setSmsKey(""); }, "এসএমএস সেটিংস সংরক্ষণ করা হয়েছে।");
  const runSmsDiagnostic = async () => { try { const result: any = await apiClient.get("/institution/sms-diagnostic"); setSmsDiagnostic(result.diagnosis || result); } catch (err: any) { setSmsDiagnostic({ verdict: `Error: ${err?.message}` }); } };
  const sendSmsTest = async () => { if (!testPhone) return; setSmsTestResult("পাঠানো হচ্ছে..."); try { const result: any = await apiClient.post("/institution/sms-test", { phone: testPhone }); setSmsTestResult(result.sent ? "✅ এসএমএস সফলভাবে পাঠানো হয়েছে" : "❌ এসএমএস পাঠানো যায়নি"); } catch (err: any) { setSmsTestResult(`❌ Error: ${err?.message}`); } };
  const toggleWeeklyDay = (day: string) => setHoliday((current) => ({ ...current, weeklyClosedDays: current.weeklyClosedDays.includes(day) ? current.weeklyClosedDays.filter((item) => item !== day) : [...current.weeklyClosedDays, day] }));
...
function ColorField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="space-y-2 rounded-lg border p-3"><span className="block text-sm font-medium">{label}</span><div className="flex items-center gap-2"><input type="color" value={value} onChange={(e) => onChange(e.target.value)} className="h-9 w-12 rounded border" /><input className="h-9 min-w-0 flex-1 rounded border px-2 text-xs" value={value} onChange={(e) => onChange(e.target.value)} /></div></label>; }
