...
const RECOMMENDED_GATEWAY_URL = "https://gateway-client-rho.vercel.app/";

const providerOptions = [
  { id: "recommended_gateway", label: "Recommended Gateway", note: `Easy School recommends ${RECOMMENDED_GATEWAY_URL}. Money will go to the school configured account.` },
...
export function PaymentSettingsPanel() {
  ...
  const [recommendedGateway, setRecommendedGateway] = useState({ accountId: "", endpoint: RECOMMENDED_GATEWAY_URL, apiKey: "", secretKey: "" });
  ...
  useEffect(() => {
    apiClient.get("/site-settings/site-config", { skipToast: true }).then((res: any) => {
      ...
      if (cfg.recommendedGateway) setRecommendedGateway((x) => ({ ...x, ...cfg.recommendedGateway, endpoint: cfg.recommendedGateway.endpoint || RECOMMENDED_GATEWAY_URL, apiKey: "", secretKey: "" }));
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        bkash,
        nagad,
        recommendedGateway: { ...recommendedGateway, endpoint: recommendedGateway.endpoint || RECOMMENDED_GATEWAY_URL },
        manualBank,
        custom,
        transactionOwner: "school",
        siteCommissionEnabled: false,
        recommendedGatewayUrl: RECOMMENDED_GATEWAY_URL,
      };
      await apiClient.put("/site-settings/site-config", { paymentGatewaySettings: payload });
      setStatus("✅ Payment settings saved. Recommended gateway URL is set. Secret values are not shown after saving.");
    } catch (e: any) { setStatus(`❌ ${e?.message || "Payment settings save failed"}`); }
    finally { setSaving(false); }
  };

  return <Card className="border-2 border-violet-200">
    <CardHeader><CardTitle className="flex items-center gap-2"><CreditCard className="h-5 w-5 text-violet-700" />School Payment System</CardTitle><CardDescription>প্রতি স্কুল নিজের payment system চালু করবে। bKash/Nagad/manual/custom credential Head গোপনে save করবে। টাকা স্কুলের account-এ যাবে; Easy School recommended gateway হিসেবে নিচের gateway suggest করবে।</CardDescription></CardHeader>
    <CardContent className="space-y-5">
      <div className="rounded-lg border border-violet-200 bg-violet-50 p-4 text-sm text-violet-900"><ShieldCheck className="mb-2 h-4 w-4" /> Recommended gateway: <a className="inline-flex items-center gap-1 font-bold underline" href={RECOMMENDED_GATEWAY_URL} target="_blank" rel="noreferrer">{RECOMMENDED_GATEWAY_URL}<ExternalLink className="h-3 w-3" /></a><br />Saved gateway secrets plain text দেখানো হবে না। প্রয়োজন হলে নতুন credential দিয়ে replace করুন।</div>
