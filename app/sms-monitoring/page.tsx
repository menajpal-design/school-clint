...
export default function SmsMonitoringPage() {
  ...
  const sendMonthlyGuardianSms = async () => {
    const total = Number(data?.summary?.totalRecipients || 0);
    const ok = window.confirm(`এই মাসের SMS সব guardian-এর কাছে পাঠাবেন? মোট recipient: ${total || 'সব active guardian'}`);
    if (!ok) return;
  ...
  const sendPublishedResultSms = async () => {
    const ok = window.confirm(`এই মাসে published হওয়া result guardian-এর কাছে SMS করবেন? Month: ${month}`);
    if (!ok) return;
