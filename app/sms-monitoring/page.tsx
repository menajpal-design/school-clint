...
export default function SmsMonitoringPage() {
  ...
  const [buyMethod, setBuyMethod] = useState("manual");
  ...
  const loadTopups = async () => {
    try {
      const result: any = await apiClient.get('/sms-monitoring/purchases', { skipToast: true });
      setTopups(Array.isArray(result?.requests) ? result.requests : []);
      setUnitPrice(Number(result?.unitPrice || 0));
    } catch { setTopups([]); }
  };
  ...
  const loadData = async () => {
    if (isDenied) return;
    setLoading(true);
    try { setData(await apiClient.get(`/sms/head/monthly?month=${month}`)); }
    catch { setData(null); }
    finally { await loadTopups(); setLoading(false); }
  };

  useEffect(() => { if (!isDenied) loadData(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [month, isDenied]);

  const submitTopup = async () => {
    if (buyQty <= 0) return alert('SMS quantity must be positive');
    if (!buyPhone.trim()) return alert('Custom contact number is required');
    setBuying(true);
    try {
      await apiClient.post('/sms-monitoring/purchases', { quantity: buyQty, contactNumber: buyPhone, paymentMethod: buyMethod, notes: buyNote, unitPrice, totalAmount: buyAmount });
      setBuyNote('');
      await loadTopups();
  ...
  return <div className="min-h-screen bg-slate-50 p-3 md:p-6"><div className="mx-auto max-w-7xl space-y-5 md:space-y-6">
    ...
    <Card className="border-emerald-200 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-emerald-700"><ShoppingCart className="h-5 w-5" /> SMS Recharge</CardTitle><CardDescription>Custom quantity and contact number দিয়ে recharge request করুন।</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap gap-2">{[100, 500, 1000].map((q) => <Button key={q} size="sm" variant={buyQty === q ? 'default' : 'outline'} onClick={() => setBuyQty(q)}>{q} SMS</Button>)}</div><div className="grid gap-3 md:grid-cols-4"><input className="rounded-lg border px-3 py-2 text-sm" type="number" min={1} value={buyQty} onChange={(e) => setBuyQty(Number(e.target.value || 0))} placeholder="Quantity" /><input className="rounded-lg border px-3 py-2 text-sm" value={buyPhone} onChange={(e) => setBuyPhone(e.target.value)} placeholder="Custom contact number" /><select className="rounded-lg border px-3 py-2 text-sm" value={buyMethod} onChange={(e) => setBuyMethod(e.target.value)}><option value="manual">Manual</option><option value="mobile_banking">Mobile banking</option><option value="cash">Cash</option></select><div className="rounded-lg border bg-slate-50 px-3 py-2 text-sm font-bold">{unitPrice > 0 ? money(buyAmount) : 'Unit price not set'}</div></div><textarea className="w-full rounded-lg border px-3 py-2 text-sm" rows={2} value={buyNote} onChange={(e) => setBuyNote(e.target.value)} placeholder="Notes optional" /><Button onClick={submitTopup} disabled={buying}>{buying ? 'Submitting...' : 'Submit SMS Recharge Request'}</Button><div className="space-y-2"><h3 className="text-sm font-semibold text-slate-700">Recharge Requests</h3>{topups.length === 0 ? <p className="text-sm text-slate-500">No recharge request found.</p> : topups.slice(0, 6).map((item: any) => <div key={item._id} className="flex flex-col gap-2 rounded-lg border bg-white p-3 text-sm sm:flex-row sm:items-center sm:justify-between"><div><b>{Number(item.quantity || 0).toLocaleString()} SMS</b> · {item.contactNumber || '-'} · {money(item.totalAmount)}<div className="text-xs text-slate-500">{formatDate(item.createdAt)}</div></div><Badge variant={item.status === 'rejected' ? 'destructive' : item.status === 'approved' || item.status === 'paid' ? 'default' : 'outline'}>{item.status}</Badge></div>)}</div></CardContent></Card>
