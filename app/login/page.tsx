...
function schoolContext() {
  if (typeof window === "undefined") return { hostname: "", subdomain: "" };
  const hostname = window.location.hostname;
  const mainDomain = process.env.NEXT_PUBLIC_MAIN_DOMAIN || "easyschool.live";
  return { hostname, subdomain: getSubdomain(hostname, mainDomain) || "" };
}

function schoolHeaders() {
  const ctx = schoolContext();
  const headers: Record<string, string> = {};
  if (ctx.subdomain) headers["x-school-subdomain"] = ctx.subdomain;
  if (ctx.hostname) headers["x-forwarded-host"] = ctx.hostname;
  return headers;
}

async function fetchCsrfToken() {
  try {
    const response = await fetch(`${API_URL}/csrf/token`, { method: "GET", credentials: "include", headers: schoolHeaders() });
...
async function loginSafely(payload: { identifier: string; password: string }) {
  ...
  const ctx = schoolContext();
  const headers: Record<string, string> = { "Content-Type": "application/json", ...schoolHeaders() };
  if (csrfToken) headers[process.env.NEXT_PUBLIC_CSRF_HEADER_NAME || "x-csrf-token"] = csrfToken;
  const response = await fetch(`${API_URL}/auth/login`, {
    method: "POST",
    headers,
    credentials: "include",
    body: JSON.stringify({ ...payload, subdomain: ctx.subdomain, domain: ctx.hostname }),
  });
  const data = await readJsonOrText(response);
  if (!response.ok) {
    throw {
