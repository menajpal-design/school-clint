...
export default function ProfilePage() {
  ...
  const studentProfile = card?.student || owner?.student || (cleanRole(owner?.role) === "student" ? roleDetails : null) || owner?.studentId || owner?.studentProfile || {};
  const teacherProfile = owner?.teacher || (["teacher", "class_teacher", "subject_teacher", "head", "assistant_head"].includes(cleanRole(owner?.role)) ? roleDetails : null) || owner?.teacherId || owner?.teacherProfile || {};
  const staffProfile = owner?.staff || (["staff", "finance_officer", "librarian"].includes(cleanRole(owner?.role)) ? roleDetails : null) || owner?.staffId || owner?.staffProfile || {};
  const parentProfile = owner?.parent || (cleanRole(owner?.role) === "parent" ? roleDetails : null) || owner?.guardian || owner?.parentProfile || {};
  ...
  const cardData = useMemo(() => {
    ...
    const className = getNested(roleProfile?.classId?.name, roleProfile?.className, owner?.classId?.name, owner?.className, owner?.student?.classId?.name, owner?.studentId?.classId?.name, personalCard?.className, cardRecord?.className);
    const sectionName = getNested(roleProfile?.sectionId?.name, roleProfile?.sectionName, owner?.sectionId?.name, owner?.sectionName, owner?.student?.sectionId?.name, owner?.studentId?.sectionId?.name, personalCard?.sectionName, cardRecord?.sectionName);
    const rollNumber = getNested(roleProfile?.rollNumber, owner?.rollNumber, owner?.student?.rollNumber, owner?.studentId?.rollNumber, personalCard?.rollNumber, cardRecord?.rollNumber);
    const employeeId = getNested(roleProfile?.employeeId, owner?.employeeId, owner?.teacher?.employeeId, owner?.staff?.employeeId, personalCard?.employeeId, cardRecord?.employeeId);
    const designation = getNested(roleProfile?.designation, owner?.designation, personalCard?.designation, cardRecord?.designation, roleLabel(role));
    const department = getNested(roleProfile?.department, owner?.department, personalCard?.department, cardRecord?.department, role === "student" ? className : "");
    ...
    const studentIdNumber = getNested(rollNumber, roleProfile?.admissionNumber, roleProfile?.registrationNumber, owner?.admissionNumber, personalCard?.cardNumber, cardRecord?.cardNumber);
    const idNumber = role === "student" ? (studentIdNumber || "-") : getNested(employeeId, personalCard?.cardNumber, cardRecord?.cardNumber, owner?.username, owner?._id, "ID");
    return { role, roleProfile, className, sectionName, rollNumber, idNumber, stream, designation, department, admissionNumber: getNested(roleProfile?.admissionNumber, owner?.admissionNumber, personalCard?.admissionNumber, cardRecord?.admissionNumber), registrationNumber: getNested(roleProfile?.registrationNumber, owner?.registrationNumber, personalCard?.registrationNumber, cardRecord?.registrationNumber), dateOfBirth: getNested(roleProfile?.dateOfBirth, owner?.dateOfBirth, personalCard?.dateOfBirth, cardRecord?.dateOfBirth), fatherName: getNested(roleProfile?.fatherName, owner?.fatherName, personalCard?.fatherName, cardRecord?.fatherName), motherName: getNested(roleProfile?.motherName, owner?.motherName, personalCard?.motherName, cardRecord?.motherName), guardianName: getNested(roleProfile?.guardianName, owner?.guardianName, personalCard?.guardianName, cardRecord?.guardianName), guardianPhone: getNested(roleProfile?.guardianPhone, owner?.guardianPhone, personalCard?.guardianPhone, cardRecord?.guardianPhone), bloodGroup: getNested(roleProfile?.bloodGroup, owner?.bloodGroup), address: getNested(roleProfile?.address, owner?.address), joiningDate: getNested(roleProfile?.joiningDate, owner?.joiningDate), salary: getNested(roleProfile?.salary, owner?.salary) };
  }, [owner, studentProfile, teacherProfile, staffProfile, parentProfile, personalCard, cardRecord]);

  const hasStudentDetails = cardData.role !== 'student' || Boolean(cardData.className || cardData.sectionName || cardData.rollNumber || cardData.guardianPhone || cardData.fatherName || cardData.motherName);
  ...
  return <div className="space-y-5"><PageHeader title="My Profile" description="Personal account, contact, institution and ID card information." icon={UserRound} actions={[{ label: "Edit Profile", icon: Edit, href: "/profile" }, { label: "My ID Card", icon: CreditCard, href: "/id-cards/my-card" }, { label: "Change Password", icon: KeyRound, href: "/profile/change-password" }]} />
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]"><section className="rounded-lg border border-border bg-card p-5 text-center shadow-sm"><div className="mx-auto flex h-28 w-28 items-center justify-center overflow-hidden rounded-full bg-slate-100">{(editing ? avatar : user?.avatar) ? <img src={editing ? avatar : user.avatar} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-12 w-12 text-slate-500" />}</div>{editing ? <div className="mt-4 space-y-3 text-left"><input className="h-10 w-full rounded-md border px-3 text-sm" value={name} onChange={(event) => setName(event.target.value)} placeholder="Full name" /><input className="h-10 w-full rounded-md border px-3 text-sm" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Phone" /><label className="block"><span className="text-sm font-medium text-slate-700">Profile image</span><input className="mt-2 block w-full text-sm" type="file" accept="image/*" onChange={(event) => uploadAvatar(event.target.files?.[0])} /></label><div className="flex gap-2"><Button type="button" onClick={saveProfile}>Save</Button><Button type="button" variant="outline" onClick={() => setEditing(false)}>Cancel</Button></div></div> : <h2 className="mt-4 text-xl font-semibold text-slate-950">{user?.name || "User"}</h2>}<p className="mt-1 text-sm capitalize text-slate-500">{roleLabel(user?.role)}</p><div className="mt-5 grid gap-2"><Button type="button" variant="outline" onClick={() => setEditing(true)}><Edit className="mr-2 h-4 w-4" />Edit Profile</Button><Button asChild><Link href="/id-cards/my-card"><CreditCard className="mr-2 h-4 w-4" />My ID Card</Link></Button><Button asChild variant="outline"><Link href="/profile/change-password"><KeyRound className="mr-2 h-4 w-4" />Change Password</Link></Button></div></section>
      <section className="space-y-5"><div className="rounded-lg border border-border bg-card p-5 shadow-sm"><h2 className="font-semibold text-foreground">Contact Info</h2><div className="mt-4 grid gap-3 md:grid-cols-2"><Info label="Username" value={user?.username} /><Info label="Email" value={user?.email || "Not set"} /><Info label="Phone" value={user?.phone || "Not set"} /><Info label="Role" value={roleLabel(user?.role)} /><Info label="Permissions" value={`${user?.permissions?.length || 0} assigned`} /></div></div>
        <div className="rounded-lg border border-border bg-card p-5 shadow-sm"><h2 className="font-semibold text-foreground">Role Details</h2>{cardData.role === 'student' && !hasStudentDetails && <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700">Student record is not linked with this login yet, so class/guardian details are unavailable. Please sync this login user with the student record.</div>}<div className="mt-4 grid gap-3 md:grid-cols-2">{cardData.role === 'student' ? <><Info label="Class" value={displayOrDash(cardData.className)} /><Info label="Section" value={displayOrDash(cardData.sectionName)} /><Info label="Roll" value={displayOrDash(cardData.rollNumber)} /><Info label="Admission No" value={displayOrDash(cardData.admissionNumber)} /><Info label="Registration No" value={displayOrDash(cardData.registrationNumber)} /><Info label="Date of Birth" value={fmtDate(cardData.dateOfBirth)} /><Info label="Blood Group" value={displayOrDash(cardData.bloodGroup)} /><Info label="Father Name" value={displayOrDash(cardData.fatherName)} /><Info label="Mother Name" value={displayOrDash(cardData.motherName)} /><Info label="Guardian Name" value={displayOrDash(cardData.guardianName)} /><Info label="Guardian Phone" value={displayOrDash(cardData.guardianPhone)} /><Info label="Address" value={displayOrDash(cardData.address)} /></> : <><Info label="ID / Employee ID" value={displayOrDash(cardData.idNumber)} /><Info label="Designation" value={displayOrDash(cardData.designation)} /><Info label="Department" value={displayOrDash(cardData.department)} /><Info label="Joining Date" value={fmtDate(cardData.joiningDate)} /><Info label="Salary" value={cardData.salary ? `৳ ${cardData.salary}` : '-'} /></>}</div></div>
