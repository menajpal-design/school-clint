...
export default function MyCardPage() {
  ...
  const cardType = owner?.role === 'head' ? 'head' : card?.ownerType || 'student';
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div ref={previewRef}>
          <IDCardPreview type={cardType as any} name={owner.name || "Your Name"} id={card?.cardNumber || "ID-CARD"} qrData={card?.qrCodeData || card?.cardNumber || ""} barcode={card?.barcodeData || card?.cardNumber || ""} role={owner?.role || cardType} />
        </div>
