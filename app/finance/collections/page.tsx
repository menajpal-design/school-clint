...
import { CreditCard, Download, Printer, ScanLine, Search } from "lucide-react";
...
export default function CollectionsPage() {
  ...
  const load = async () => { setLoading(true); setError(""); try { const data = await financeApi.collections({ search }) as any; setStudents(data.students || []); } catch (err: any) { setError(err?.message || "Failed to load students."); } finally { setLoading(false); } };
  ...
  const collect = async () => {
    ...
    try { const data = await financeApi.collectPayment({ studentId: selected._id, amount, paymentMethod, notes }) as any; setReceipt(data.payment); setMessage("Offline fee collected successfully. Receipt is ready and student profile history updated."); await load(); setSelected((current: any) => current ? { ...current, dueAmount: Math.max(0, Number(current.dueAmount || 0) - Number(amount || 0)) } : current); }
    catch (err: any) { setError(err?.message || "Failed to collect payment."); }
    finally { setSaving(false); }
  };
  const choose = (student: any) => { setSelected(student); setAmount(Math.max(0, Number(student.dueAmount || 0))); setReceipt(null); setMessage(""); setError(""); };
  const printReceipt = async () => {
    ...
    await printHtml("Professional Fee Receipt", `<main class="print-card"><p class="print-title">Official Fee Collection Receipt</p><p class="print-muted">Receipt No: ${receipt.receiptNumber || "-"}</p><div class="print-grid"><div class="print-row"><strong>Student</strong>${studentName}</div><div class="print-row"><strong>Roll</strong>${roll}</div><div class="print-row"><strong>Class</strong>${cls}</div><div class="print-row"><strong>Section</strong>${section}</div><div class="print-row"><strong>Paid Amount</strong>${formatCurrency(receipt.amount || 0)}</div><div class="print-row"><strong>Payment Method</strong>${receipt.paymentMethod || paymentMethod || "cash"}</div><div class="print-row"><strong>Payment Date</strong>${formatDate(receipt.paymentDate || new Date())}</div><div class="print-row"><strong>Collected By</strong>${receipt.collectedBy?.name || "School Office"}</div><div class="print-row"><strong>Note</strong>${receipt.notes || notes || "Offline fee collection"}</div></div><div class="signature"><div>Accounts / Class Teacher</div><div>Guardian Signature</div></div></main>`, "", JSON.stringify({ type: "fee_receipt", receiptNumber: receipt.receiptNumber, student: studentName, amount: receipt.amount, date: receipt.paymentDate || new Date() }));
  };
