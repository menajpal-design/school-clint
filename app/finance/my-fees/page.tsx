...
export default function MyFeesPage() {
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-lg border border-border bg-card shadow-sm"><Table><TableHeader><TableRow className="bg-muted hover:bg-muted"><TableHead>Receipt</TableHead><TableHead>Amount</TableHead><TableHead>Method</TableHead><TableHead>Date</TableHead><TableHead></TableHead></TableRow></TableHeader><TableBody>{visiblePayments.map((p) => <TableRow key={p._id}><TableCell>{p.receiptNumber}</TableCell><TableCell>{formatCurrency(p.amount || 0)}</TableCell><TableCell>{p.paymentMethod}</TableCell><TableCell>{formatDate(p.paymentDate)}</TableCell><TableCell><Button size="sm" variant="outline" onClick={() => printReceipt(p)}><Download className="mr-2 h-4 w-4" />Receipt</Button></TableCell></TableRow>)}</TableBody></Table></section>
  </div>;
}
