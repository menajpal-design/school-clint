...
export default function FinanceReportsPage() {
  ...
  const classWiseRows = useMemo(() => {
    ...
    (reports.collections || []).forEach((row: any) => {
      ...
      const item = map.get(name) || { _id: name, className: name, count: 0, amount: 0 };
      item.count += 1;
  ...
  return <div className="space-y-5">
    ...
    <div ref={reportRef} className="space-y-5 bg-card">
      ...
      <ClassCollectionTable rows={classWiseRows} loading={loading} />
...
function ClassCollectionTable({ rows, loading }: { rows: any[]; loading: boolean }) {
  const [page, setPage] = useState(1);
  const pageSize = 5;
  const totalPages = Math.max(1, Math.ceil(rows.length / pageSize));
  const visible = rows.slice((page - 1) * pageSize, page * pageSize);
  return <section className="overflow-hidden rounded-lg border border-border bg-card shadow-sm"><div className="border-b border-border p-4 font-semibold">Class-wise Collection Summary</div><Table><TableHeader><TableRow className="bg-muted hover:bg-muted"><TableHead>Class</TableHead><TableHead>Total Receipts</TableHead><TableHead>Total Collection</TableHead></TableRow></TableHeader><TableBody>{loading ? <TableRow><TableCell colSpan={3} className="h-24 text-center text-muted-foreground">Loading records...</TableCell></TableRow> : rows.length === 0 ? <TableRow><TableCell colSpan={3} className="h-24 text-center text-muted-foreground">No class-wise collection found.</TableCell></TableRow> : visible.map((r:any)=><TableRow key={r.className}><TableCell>{r.className}</TableCell><TableCell>{r.count}</TableCell><TableCell>{formatCurrency(r.amount || 0)}</TableCell></TableRow>)}</TableBody></Table>{rows.length > pageSize && <Pager page={page} totalPages={totalPages} onPage={setPage} />}</section>;
}
