...
const cqQuestions = (count: number, subject: string, syllabus: string): QuestionItem[] => Array.from({ length: Math.max(1, Math.min(60, count || 8)) }).map((_, index) => ({ type: 'cq', question: `${index + 1}. ${subject || 'Subject'} - ${syllabus || 'সিলেবাস'} থেকে একটি সৃজনশীল/CQ প্রশ্ন তৈরি করুন।\nক) জ্ঞানমূলক প্রশ্ন\nখ) অনুধাবনমূলক প্রশ্ন\nগ) প্রয়োগমূলক প্রশ্ন\nঘ) উচ্চতর দক্ষতামূলক প্রশ্ন`, options: [], answer: 'Teacher verification required', marks: 10 }));
...
export default function AiManagePage() {
  ...
  const printCopy = () => {
    ...
    win.document.write(`<html><head><title>${form.title}</title><style>@page{size:A4;margin:14mm}body{font-family:Arial,sans-serif;line-height:1.55;color:#111}.head{text-align:center;margin-bottom:18px}.q{border:1px solid #ddd;padding:10px;margin:10px 0;break-inside:avoid;white-space:pre-wrap}.meta{font-size:13px}</style></head><body><div class="head"><h2>${form.title}</h2><p class="meta">${form.className} · ${form.subjectName} · ${form.duration} · Marks: ${form.totalMarks}</p><b>CQ / Creative Question Paper</b></div>${questions.map((q, i) => `<div class="q"><b>${i + 1}. ${String(q.question || '').replace(/^\d+\.\s*/, '')}</b><br/><br/>Marks: ${q.marks || 10}<br/>Note: ${q.answer || 'Teacher verification required'}</div>`).join('')}<script>window.onload=function(){setTimeout(function(){window.print()},300)}<\/script></body></html>`);
    win.document.close();
  };

  return <div className="space-y-6 p-4 md:p-6"><div className="rounded-2xl border bg-gradient-to-r from-indigo-50 via-white to-emerald-50 p-5 shadow-sm"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><h1 className="text-3xl font-bold tracking-tight">CQ Question Manage</h1><p className="text-sm text-muted-foreground">Gemini/server generator দিয়ে class-wise syllabus অনুযায়ী CQ/Creative question set তৈরি, save, print ও download করুন।</p></div><div className="flex flex-wrap gap-2"><Badge>Teacher</Badge><Badge>Head</Badge><Badge>CQ</Badge></div></div></div><div className="grid gap-4 xl:grid-cols-[420px_1fr]"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5" />CQ Setup</CardTitle><CardDescription>Class, subject, syllabus ও CQ count দিন।</CardDescription></CardHeader><CardContent className="space-y-3"><Input value={form.title} onChange={(e) => update('title', e.target.value)} placeholder="Question set title" /><div className="grid gap-3 sm:grid-cols-2"><Input value={form.className} onChange={(e) => update('className', e.target.value)} placeholder="Class" /><Input value={form.subjectName} onChange={(e) => update('subjectName', e.target.value)} placeholder="Subject" /></div><div className="grid gap-3 sm:grid-cols-3"><Input value={form.count} onChange={(e) => update('count', e.target.value)} placeholder="CQ Count" /><Input value={form.totalMarks} onChange={(e) => update('totalMarks', e.target.value)} placeholder="Total marks" /><Input value={form.duration} onChange={(e) => update('duration', e.target.value)} placeholder="Duration" /></div><Input value={form.language} onChange={(e) => update('language', e.target.value)} placeholder="Language" /><textarea className="min-h-36 w-full rounded-md border bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-primary/20" value={form.syllabus} onChange={(e) => update('syllabus', e.target.value)} placeholder="Syllabus / chapter / learning outcome / topic" /><div className="grid gap-2 sm:grid-cols-2"><Button onClick={generate} disabled={loading}>{loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <BookOpenCheck className="mr-2 h-4 w-4" />}Generate CQ</Button><Button variant="outline" onClick={saveSet} disabled={saving || !questions.length}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save to Server</Button></div></CardContent></Card><div className="space-y-4"><div className="grid gap-3 md:grid-cols-3"><Card><CardContent className="p-4"><p className="text-xs text-muted-foreground">CQ Questions</p><p className="text-2xl font-bold">{questions.length}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-xs text-muted-foreground">Type</p><p className="text-2xl font-bold">CQ</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-xs text-muted-foreground">Marks</p><p className="text-2xl font-bold">{totalMarks || form.totalMarks}</p></CardContent></Card></div><Card><CardHeader><CardTitle className="flex items-center gap-2"><Database className="h-5 w-5" />Generated CQ Output</CardTitle><CardDescription>Review, copy, download, print, তারপর server storage-এ save করুন।</CardDescription></CardHeader><CardContent className="space-y-3">{!questions.length ? <p className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">Generate CQ চাপলে preview এখানে দেখাবে।</p> : <div className="max-h-[620px] space-y-3 overflow-auto rounded-lg border bg-muted/30 p-3">{questions.map((item, index) => <div key={index} className="rounded-lg bg-background p-3 shadow-sm"><p className="whitespace-pre-wrap font-medium">{index + 1}. {String(item.question || '').replace(/^\d+\.\s*/, '')}</p><div className="mt-2 flex gap-2 text-xs"><Badge variant="secondary">CQ</Badge><Badge variant="outline">Marks {item.marks || 10}</Badge>{item.answer && <Badge>{item.answer}</Badge>}</div></div>)}</div>}{questions.length > 0 && <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => navigator.clipboard.writeText(text)}><Copy className="mr-2 h-4 w-4" />Copy</Button><Button variant="outline" onClick={() => downloadFile('cq-question-set.txt', text)}><Download className="mr-2 h-4 w-4" />Download TXT</Button><Button variant="outline" onClick={() => downloadFile('cq-question-set.json', JSON.stringify({ ...form, questions }, null, 2), 'application/json')}><FileJson className="mr-2 h-4 w-4" />Download JSON</Button><Button variant="outline" onClick={printCopy}><Printer className="mr-2 h-4 w-4" />Print</Button></div>}</CardContent></Card></div></div></div>;
}
