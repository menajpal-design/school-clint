...
export default function ClassRoutinePage() {
  ...
  const selectedChild = useMemo(() => role === "parent" ? children.find((c) => c._id === selectedChildId) || children[0] : null, [role, children, selectedChildId]);
  ...
  const displayRoutines = useMemo(() => {
    ...
    if (role === "parent" && selectedChild) {
      ...
      const childSectionId = String(selectedChild.sectionId?._id || selectedChild.sectionId || "");
      list = routines.filter((routine) => {
        const routineClassId = String(routine.classId?._id || routine.classId || "");
        const routineSectionId = String(routine.sectionId?._id || routine.sectionId || "");
        const matchClass = routineClassId === childClassId;
        const matchSection = !routineSectionId || routineSectionId === childSectionId;
        return matchClass && matchSection;
      });
    }
    return list.filter((routine) => !statusFilter || routine.status === statusFilter);
  }, [routines, role, selectedChild, statusFilter]);

  const selectedClass = useMemo(() => {
    if (role === "parent" && selectedChild) {
      return selectedChild.classId;
    }
    return classOptions.find((item) => item._id === classId);
  }, [role, selectedChild, classOptions, classId]);
  ...
  const grouped = days.map((day) => ({ day, items: displayRoutines.filter((routine) => routine.dayOfWeek === day).sort((a, b) => String(a.startTime).localeCompare(String(b.startTime))) }));
  const printPeriods = useMemo(() => {
    const fromData = Array.from(new Set(displayRoutines.map((item) => item.periodName || "1")));
  ...
  const loadRoutines = useCallback(async () => {
    ...
    try {
      ...
      if (data.children) {
        ...
        setSelectedChildId((current) => current || data.children?.[0]?._id || "");
      }
  ...
  const openPreviewEditor = () => displayRoutines.length > 0 ? openEdit(displayRoutines[0]) : openCreate();
  ...
  const getCell = (day: string, period: string) => displayRoutines.find((routine) => routine.dayOfWeek === day && String(routine.periodName || "").toLowerCase() === String(period).toLowerCase());

  return <div className="space-y-5">
    <PageHeader title="Class Routine" description={isViewOnly ? "View and download published routine." : "Create, edit, approve and publish class routines."} icon={CalendarDays} status={<Badge variant="outline">{displayRoutines.length} routine items</Badge>} actions={[
