...
export default function ExamRoutinePage() {
  ...
  return <div className="space-y-5">
    ...
    <section className="no-print rounded-lg border bg-card p-4 shadow-sm"><h2 className="font-semibold">Saved Routine List</h2><p className="mb-3 text-xs text-muted-foreground">Preview button চাপলে dialog box-এ routine দেখা যাবে।</p><div className="overflow-x-auto"><table className="w-full min-w-[1180px] table-fixed text-sm"><thead><tr className="border-b bg-slate-50 text-left"><th className="w-[180px] p-2">Exam</th><th className="w-[130px] p-2">Class</th><th className="w-[210px] p-2">Date</th><th className="w-[80px] p-2">Subjects</th><th className="w-[110px] p-2">Routine</th><th className="w-[100px] p-2">Publish</th><th className="w-[360px] p-2 text-right">Action</th></tr></thead><tbody>{saved.length === 0 ? <tr><td colSpan={7} className="p-6 text-center text-muted-foreground">No saved routine found.</td></tr> : saved.map((e) => <tr key={idOf(e)} className={idOf(e) === idOf(selectedExam) ? "border-b bg-blue-50" : "border-b"}><td className="p-2"><div className="break-words font-medium">{e.name}</div><div className="text-xs text-muted-foreground">{e.type || "term"}</div></td><td className="p-2">{e.classId?.name || e.className || "-"}</td><td className="p-2">{fmt(e.startDate)} {e.endDate ? `- ${fmt(e.endDate)}` : ""}</td><td className="p-2">{e.subjectMarks?.length || 0}</td><td className="p-2"><Badge variant="outline" className={ready(e) ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"}>{ready(e) ? "Ready" : "Incomplete"}</Badge></td><td className="p-2"><Badge variant="outline" className={e.isPublished ? "border-blue-200 bg-blue-50 text-blue-700" : "border-slate-200 bg-slate-50 text-slate-600"}>{e.isPublished ? "Public" : "Private"}</Badge></td><td className="p-2"><div className="flex flex-wrap justify-end gap-2"><Button size="sm" variant="outline" onClick={() => select(e)}>Edit</Button><Button size="sm" variant="outline" onClick={() => setPreview(e)}>Preview</Button><Button size="sm" variant={e.isPublished ? "outline" : "default"} disabled={saving} onClick={() => publish(e)}>{e.isPublished ? "Unpublish" : "Publish"}</Button><Button size="sm" variant="outline" onClick={() => download(e)}><Download className="mr-1 h-3.5 w-3.5" />Download</Button></div></td></tr>)}</tbody></table></div></section>
    {canManage && <section className="no-print rounded-lg border bg-card p-4 shadow-sm"><div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">Edit Exam Routine</h2><Button size="sm" variant="outline" onClick={addRow}><Plus className="mr-2 h-4 w-4" />Add Subject Row</Button></div><div className="overflow-x-auto"><table className="w-full min-w-[860px] text-sm"><thead><tr className="border-b text-left"><th className="p-2">Subject</th><th className="p-2">Date</th><th className="p-2">Duration</th><th className="p-2">Full</th><th className="p-2">Pass</th><th className="p-2">Action</th></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={6} className="p-4 text-center text-muted-foreground">No subject routine found.</td></tr> : rows.map((r, i) => <tr key={i} className="border-b"><td className="p-2"><select className="h-9 min-w-[220px] rounded-md border px-2" value={r.subjectId} onChange={(ev) => { const s = classSubjects.find((x: any) => x._id === ev.target.value); setRows((a) => a.map((row, j) => j === i ? { ...row, subjectId: ev.target.value, subjectName: s?.name || row.subjectName, subjectCode: s?.code || row.subjectCode } : row)); }}><option value="">Select subject</option>{classSubjects.map((s: any) => <option key={s._id} value={s._id}>{s.name}</option>)}</select></td><td className="p-2"><input type="date" className="h-9 rounded-md border px-2" value={r.date} onChange={(ev) => setRows((a) => a.map((row, j) => j === i ? { ...row, date: ev.target.value } : row))} /></td>{(["duration", "totalMarks", "passingMarks"] as const).map((k) => <td key={k} className="p-2"><input type="number" className="h-9 w-24 rounded-md border px-2" value={r[k]} onChange={(ev) => setRows((a) => a.map((row, j) => j === i ? { ...row, [k]: Number(ev.target.value) } : row))} /></td>)}<td className="p-2"><Button size="sm" variant="destructive" onClick={() => setRows((a) => a.filter((_, j) => j !== i))}><Trash2 className="h-4 w-4" /></Button></td></tr>)}</tbody></table></div></section>}
