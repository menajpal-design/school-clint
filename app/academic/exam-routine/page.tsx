...
type ExamRoutine = { _id?: string; id?: string; name: string; type?: string; classId?: any; className?: string; sectionId?: any; startDate?: string; endDate?: string; status?: string; isPublished?: boolean; subjectMarks?: any[]; instructions?: string; syllabus?: string; approvalRequired?: boolean; updatedAt?: string };
...
export default function ExamRoutinePage() {
  ...
  const selectRoutine = (exam: ExamRoutine, quiet = false) => {
    ...
    syncEditor(exam);
    if (!quiet) showInfo(`Selected routine: ${exam.name}`, "Routine selected");
  };
  const openSavedRoutine = (exam: ExamRoutine) => selectRoutine(exam, false);

  const publishSavedRoutine = async (exam: ExamRoutine) => {
    if (!exam?._id && !exam?.id) return showError("❌ Routine ID পাওয়া যায়নি।");
    if (!exam.isPublished && !isRoutineReady(exam)) return showError("❌ Routine incomplete. Publish করার আগে subject, date, duration complete করুন।");
    setSaving(true); setError(""); setMessage("");
    try {
      const nextPublished = !exam.isPublished;
      const result: any = await apiClient.patch(`/academic/exams/${exam._id || exam.id}/public-routine`, { isPublished: nextPublished });
      const updatedExam = result?.exam || { ...exam, isPublished: nextPublished, updatedAt: new Date().toISOString() };
      setExams((current) => {
        const next = uniqueById([updatedExam, ...current.filter((item: any) => String(item._id || item.id) !== String(exam._id || exam.id))]);
        writeJson(EXAM_CACHE_KEY, next);
        return next;
      });
      showSuccess(nextPublished ? "✅ Routine published হয়েছে।" : "✅ Routine unpublished/private হয়েছে।", "Routine publish updated");
