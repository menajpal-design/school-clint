...
const BackRow = ({ label, value }: { label: string; value?: string }) => value ? (
  <div style={{ display: 'grid', gridTemplateColumns: '62px 1fr', gap: 6, alignItems: 'start', minHeight: 18, fontSize: 8.5, lineHeight: '11px', color: '#111827' }}>
...
export const ProfessionalIDCard = React.forwardRef<HTMLDivElement, ProfessionalIDCardProps>((props, ref) => {
  ...
  const roleDetail = normalizedRole === 'student' ? (stream || department || 'Student') : (designation || department || stream || displayRole)
  ...
  const resolvedQrData = qrData || JSON.stringify({ name, id: idNumber, roll: rollNumber, role: displayRole, institution: institutionName, validity: displayValidityDate, class: stream, designation, department })

  return (
    <div ref={ref} className={`professional-id-card ${className}`} style={{ position: 'relative', width: 800, height: 500, minWidth: 800, maxWidth: 800, minHeight: 500, maxHeight: 500, boxSizing: 'border-box', overflow: 'hidden', background: '#ffffff', padding: 0, flex: '0 0 auto', marginBottom: 18 }}>
      <div style={{ display: 'flex', gap: 30, alignItems: 'stretch', justifyContent: 'center', flexWrap: 'nowrap', width: 800, height: 500, paddingTop: 0 }}>
        <section style={{ position: 'relative', width: 380, height: 500, overflow: 'hidden', background: theme.leftBg, fontFamily: 'Arial, Helvetica, sans-serif', color: '#ffffff' }}>
          ...
          <div style={{ position: 'absolute', left: 25, right: 25, top: 306, border: '1px solid rgba(255,255,255,0.9)', background: 'rgba(255,255,255,0.03)', paddingBottom: 10 }}>
            <Row label="Name:" value={name} />
            <Row label="Role:" value={displayRole} />
            <Row label={leftIdLabel} value={displayId} />
            <Row label={`${backDetailLabel}:`} value={roleDetail} withBottomBorder={false} />
        ...
        <section style={{ position: 'relative', width: 380, height: 500, overflow: 'hidden', background: '#ffffff', fontFamily: 'Arial, Helvetica, sans-serif', color: '#082f3a', flex: '0 0 auto' }}>
          ...
          <div style={{ position: 'absolute', top: 30, left: 30 }}><div style={{ maxWidth: 160, color: theme.gold, fontFamily: 'Georgia, serif', fontSize: 20, fontWeight: 800, lineHeight: '20px', overflowWrap: 'anywhere', wordBreak: 'break-word' }}>{institutionName}</div></div>
          <div style={{ position: 'absolute', top: 30, right: 35 }}><LogoMark logoUrl={institutionSeal || institutionLogo} institutionName={institutionName} tone={theme.gold} /></div>
          <div style={{ position: 'absolute', left: 15, top: 112, width: 320, display: 'grid', gap: 6 }}>
            ...
            <BackRow label="Role" value={displayRole} />
            <BackRow label="Roll" value={rollNumber} />
            <BackRow label={backDetailLabel} value={roleDetail} />
          ...
          <div style={{ position: 'absolute', left: 50, top: 280, width: 100, height: 100, background: '#ffffff' }}><QRCodeSVG value={resolvedQrData} size={96} level="M" includeMargin={false} /></div>
          <div style={{ position: 'absolute', left: 170, top: 280, width: 150, textAlign: 'center' }}>{headSignature ? <img src={headSignature} alt="Signature" style={{ marginTop: 18, width: 88, height: 34, objectFit: 'contain' }} /> : <div style={{ margin: '28px auto 0', width: 80, height: 1, background: '#334155' }} />}</div>
