...
export async function downloadHtmlAsPdf(title: string, bodyHtml: string, styles = "", filename?: string) {
  ...
  wrapper.style.cssText = `position:fixed;left:0;top:0;width:${wrapperWidth}px;min-width:${wrapperWidth}px;min-height:${A4_MIN_HEIGHT_PX}px;background:#fff;padding:${CAPTURE_PADDING_PX}px;z-index:2147483647;pointer-events:none;overflow:visible;box-sizing:border-box;`;
  ...
  try {
    ...
    await saveCanvasAsPdf(canvas, filename || `${safeFilename(title)}.pdf`, isFixedReceipt);
...
export async function printHtml(title: string, bodyHtml: string, styles = "", qrValue?: string) {
  ...
  const popup = window.open("", "_blank", isFixedReceipt ? "width=1200,height=820" : "width=1000,height=900");
  if (!popup) { await downloadHtmlAsPdf(title, bodyWithQr, finalStyles, `${safeFilename(title)}.pdf`); return; }
  popup.document.open();
  popup.document.write(pageShell(title, isFixedReceipt ? `<div class="pdf-fixed-shell">${bodyWithQr}</div>` : bodyWithQr, finalStyles));
