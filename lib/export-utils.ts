...
export function getPrintInstitution() {
  ...
  if (typeof window !== "undefined") {
    try { storedInstitution = JSON.parse(localStorage.getItem("printInstitution") || localStorage.getItem("institution") || "null"); }
    catch { storedInstitution = null; }
  }
...
export async function makeQrDataUrl(value: string, width = 128) {
  ...
  return QRCode.toDataURL(value, { width, margin: 1, errorCorrectionLevel: "M", color: { dark: "#0f172a", light: "#ffffff" } });
}
...
const copyComputedStyles = (clone: HTMLElement, source: Element) => {
  ...
  cloneChildren.forEach((child, index) => { if (sourceChildren[index]) copyComputedStyles(child, sourceChildren[index]); });
};
...
export function downloadCsv(filename: string, rows: unknown[][]) { downloadFile(`\uFEFF${rows.map((row) => row.map(csvCell).join(",")).join("\r\n")}`, filename, "text/csv;charset=utf-8"); }

export async function downloadHtmlAsPdf(title: string, bodyHtml: string, styles = "", filename?: string) {
  const html2canvas = (await import("html2canvas")).default;
  const jsPDF = (await import("jspdf")).default;
  const wrapper = document.createElement("div");
  wrapper.style.position = "fixed"; wrapper.style.left = "0"; wrapper.style.top = "0"; wrapper.style.width = "794px"; wrapper.style.minHeight = "1123px"; wrapper.style.background = "#ffffff"; wrapper.style.padding = "45px"; wrapper.style.zIndex = "-1"; wrapper.style.opacity = "0"; wrapper.style.pointerEvents = "none";
  ...
  const canvas = await html2canvas(wrapper, { scale: 1.5, backgroundColor: "#ffffff", useCORS: true, allowTaint: true, scrollX: 0, scrollY: 0, width: 794, windowWidth: 794 });
...
export async function downloadElementPdf(target: HTMLElement | null, filename: string) {
  ...
  const forceLandscape = fullWidth >= 900 || fullWidth > fullHeight * 0.72;
  ...
  const pdf = new jsPDF(forceLandscape ? "l" : "p", "mm", "a4");
  const pageWidth = pdf.internal.pageSize.getWidth();
  const pageHeight = pdf.internal.pageSize.getHeight();
  const marginX = 3;
  const marginY = 3;
  ...
  let heightLeft = imgHeight;
  let y = marginY;
  pdf.addImage(imgData, "PNG", marginX, y, imgWidth, imgHeight, undefined, "FAST");
...
export async function printElement(target: HTMLElement | null, title = "Print") {
  ...
  popup.document.open(); popup.document.write(pageShell(title, `${styleTags}<main style="padding:20px;background:#fff;">${cloned.outerHTML}</main>`)); popup.document.close(); popup.focus();
...
export async function printHtml(title: string, bodyHtml: string, styles = "", qrValue?: string) {
  ...
  popup.document.open(); popup.document.write(pageShell(title, bodyWithQr, styles)); popup.document.close(); popup.focus();
  setTimeout(() => { try { popup.print(); } catch {} }, 500);
}
