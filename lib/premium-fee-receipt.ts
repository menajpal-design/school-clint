...
function encodeVerifyData(payload: any) {
  try { return btoa(unescape(encodeURIComponent(JSON.stringify(payload)))); } catch { return btoa(JSON.stringify(payload)); }
}
...
export async function printPremiumFeeReceipt(payment: any, student: any, institutionProfile?: any) {
  ...
  const paymentDate = payment?.paymentDate || payment?.paidAt || payment?.createdAt || new Date();
  const amount = Number(payment?.amount || 0);
  const feeType = payment?.feeId?.type || payment?.invoiceId?.feeType || payment?.type || "Student Fee";
  const monthYear = `${payment?.month || payment?.feeId?.month || payment?.invoiceId?.month || "-"} ${payment?.year || payment?.feeId?.year || payment?.invoiceId?.year || ""}`.trim();
  const collectedBy = payment?.collectedBy?.name || payment?.createdBy?.name || "Accounts Office";
  const verifyUrl = buildFeeReceiptVerifyUrl(payment, student, institution.name);
  const qrDataUrl = await makeQrDataUrl(verifyUrl, 150);
  ...
  const styles = `.premium-receipt{width:980px;max-width:none;min-width:980px;position:relative;border:1px solid #d6b25e;border-radius:22px;padding:0;overflow:hidden;background:#fff;color:#0f172a;box-shadow:0 20px 50px rgba(15,23,42,.14);box-sizing:border-box}.premium-receipt:before{content:"PAID";position:absolute;left:8%;bottom:15%;font-size:118px;font-weight:900;letter-spacing:.22em;color:rgba(22,163,74,.045);transform:rotate(-24deg);z-index:0}.premium-receipt>*{position:relative;z-index:1}.hero{display:flex;justify-content:space-between;align-items:center;gap:18px;padding:24px;background:linear-gradient(135deg,#052e2b,#0f766e 54%,#d6b25e);color:#fff}.brand-left{display:flex;gap:16px;align-items:center;min-width:0}.logo{width:78px;height:78px;border:2px solid rgba(255,255,255,.55);border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900;background:rgba(255,255,255,.12);overflow:hidden;flex:0 0 auto}.logo img{width:100%;height:100%;object-fit:contain;background:#fff;padding:6px}.eyebrow{margin:0 0 5px;text-transform:uppercase;letter-spacing:.22em;font-size:10px;font-weight:900;opacity:.88}.dark{color:#0f766e}.hero h1{margin:0;font-size:28px;line-height:1.1;color:#fff;text-transform:uppercase;max-width:690px;overflow-wrap:anywhere}.hero p{margin:4px 0 0;font-size:12px;color:rgba(255,255,255,.88);max-width:700px;overflow-wrap:anywhere}.qr{display:flex;flex-direction:column;align-items:center;gap:5px;font-size:10px;font-weight:900;text-transform:uppercase;color:#fff;flex:0 0 auto}.qr img{width:96px;height:96px;border-radius:14px;border:2px solid rgba(255,255,255,.7);background:#fff;padding:5px}.invoice-row{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;padding:22px 24px 14px;border-bottom:1px solid #e2e8f0}.invoice-row h2{margin:0;font-size:32px;line-height:1.05}.muted{margin:6px 0 0;color:#64748b;font-size:12px}.stamp{border:4px double #16a34a;color:#16a34a;border-radius:999px;padding:12px 20px;font-size:18px;font-weight:900;letter-spacing:.18em;transform:rotate(-8deg);white-space:nowrap}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding:18px 24px}.summary div{border:1px solid #ead7a1;background:linear-gradient(180deg,#fffbeb,#fff);border-radius:14px;padding:12px;min-width:0}.summary span{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.14em;color:#92400e;font-weight:900}.summary b{display:block;margin-top:5px;font-size:13px;overflow-wrap:anywhere}.cards{display:grid;grid-template-columns:1fr 1fr;gap:14px;padding:0 24px 18px}.card{border:1px solid #cbd5e1;border-radius:16px;padding:16px;background:#fff;min-width:0}.card h3{margin:0 0 12px;font-size:13px;text-transform:uppercase;letter-spacing:.16em;color:#0f766e}.card p{display:flex;justify-content:space-between;gap:12px;border-bottom:1px dashed #e2e8f0;margin:0;padding:7px 0;font-size:12px}.card p:last-child{border-bottom:0}.card b{color:#475569;white-space:nowrap}.card span{text-align:right;font-weight:700;overflow-wrap:anywhere}.amount-card{background:linear-gradient(180deg,#f0fdfa,#fff)}.amount{font-size:36px;font-weight:900;color:#052e2b;margin:4px 0}.words{display:block!important;border:0!important;color:#0f766e!important;font-style:italic;padding:0 0 10px!important}.table-wrap{padding:0 24px 18px;overflow:visible}.table-wrap table{width:100%;border-collapse:collapse;border-radius:14px;overflow:hidden;table-layout:auto}.table-wrap th{background:#052e2b;color:#fff;padding:10px;border:1px solid #052e2b;font-size:11px;white-space:normal}.table-wrap td{padding:10px;border:1px solid #cbd5e1;font-size:12px;white-space:normal;word-break:break-word}.table-wrap tfoot td{background:#fffbeb;font-weight:900}.right{text-align:right}.note{margin:0 24px 18px;border-left:5px solid #d6b25e;background:#fffbeb;padding:13px;border-radius:0 12px 12px 0;font-size:12px;color:#475569}.signatures{display:grid;grid-template-columns:repeat(3,1fr);gap:26px;padding:42px 24px 22px}.signatures div{text-align:center;font-size:12px}.signatures i{display:block;height:1px;background:#334155;margin-bottom:8px}.signatures b{display:block}.signatures small{display:block;margin-top:3px;color:#64748b}.signatures em{display:inline-flex;width:76px;height:76px;border:2px dashed #d6b25e;border-radius:999px;align-items:center;justify-content:center;font-style:normal;font-weight:900;color:#92400e;margin-bottom:8px}footer{display:flex;justify-content:space-between;gap:12px;padding:12px 24px;background:#f8fafc;color:#64748b;font-size:11px}`;
  await printHtml(`Premium Fee Receipt - ${receiptNo}`, body, styles, verifyUrl);
}
