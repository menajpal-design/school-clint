...
class ApiClient {
  ...
  setToken(value: string, persist = true) {
    ...
    if (!isBrowser) return;
    if (persist) { localStorage.setItem('token', value); sessionStorage.removeItem('token'); return; }
    sessionStorage.setItem('token', value); localStorage.removeItem('token');
  }

  getToken() { if (this.currentToken) return this.currentToken; if (!isBrowser) return null; this.currentToken = localStorage.getItem('token') || sessionStorage.getItem('token'); return this.currentToken; }
  clearToken() { this.currentToken = null; if (!isBrowser) return; localStorage.removeItem('token'); sessionStorage.removeItem('token'); }

  private headers(body?: any, extra?: any, url: string = '') {
    const h: any = { ...(extra || {}) };
    if (!(body instanceof FormData) && !h['Content-Type']) h['Content-Type'] = 'application/json';
    const auth = this.getToken(); if (auth) h.Authorization = `Bearer ${auth}`;
    if (isBrowser && this.shouldAttachInstitutionScope(url)) { const institutionId = localStorage.getItem('selectedInstitutionId'); if (institutionId) h['x-institution-id'] = institutionId; }
    return h;
  }
  ...
  private async request<T>(method: Method, url: string, data?: any, config: any = {}): Promise<T> {
    ...
    if (method !== 'GET' && method !== 'DELETE') { try { await this.ensureCsrfToken(); if (this.csrfToken) headers[this.csrfHeaderName] = this.csrfToken; } catch {} }
    const init: RequestInit = { method, headers, body, credentials: 'include' };
    try {
      let res = await withTimeout(`${API_URL}${url}${qs}`, init, 90000);
      if (res.status === 401 && isBrowser) { const refreshed = await this.attemptRefresh(); if (refreshed) res = await withTimeout(`${API_URL}${url}${qs}`, init, 90000); }
      ...
      if (res.status === 403 && isBrowser) {
        ...
        if (String(msg).toLowerCase().includes('csrf')) { this.csrfToken = null; await this.ensureCsrfToken(); if (this.csrfToken) { init.headers = { ...(init.headers || {}), [this.csrfHeaderName]: this.csrfToken }; res = await withTimeout(`${API_URL}${url}${qs}`, init, 90000); bodyText = null; } }
      }
    ...
    try {
      ...
      if (!this.csrfToken) {
        try {
          ...
          if (unique.length > 1) {
            try { const mainDomain = process.env.NEXT_PUBLIC_MAIN_DOMAIN || ''; document.cookie = `${cookieName}=;expires=Thu, 01 Jan 1970 00:00:00 GMT;path=/`; if (mainDomain) document.cookie = `${cookieName}=;expires=Thu, 01 Jan 1970 00:00:00 GMT;path=/;domain=.${mainDomain}`; } catch {}
            this.clearToken(); if (isBrowser) window.location.href = '/login'; return;
          }
  ...
  private async attemptRefresh(): Promise<boolean> { if (!isBrowser || this.refreshing) return false; this.refreshing = true; try { const res = await withTimeout(`${API_URL}/auth/refresh`, { method: 'POST', credentials: 'include' }, 15000); if (!res.ok) return false; this.csrfToken = null; await this.ensureCsrfToken(); return true; } catch { return false; } finally { this.refreshing = false; } }
  private async parse<T>(res: Response, config: any = {}, preReadText?: string | null): Promise<T> { if (res.status === 401) { this.clearToken(); if (isBrowser) window.location.href = '/login'; } const text = (preReadText !== undefined && preReadText !== null) ? preReadText : await res.text(); const data = text ? (() => { try { return JSON.parse(text); } catch { return text; } })() : null; if (!res.ok) { if (isBrowser && (res.status === 428 || data?.code === 'STORAGE_CONFIG_REQUIRED')) { const message = data?.message || 'দয়া করে MongoDB URL সেট করুন।'; this.toast(message, 'Storage configuration required'); const redirectTo = data?.redirectTo || '/settings'; if (window.location.pathname !== redirectTo) window.location.href = redirectTo; } throw this.toError(data || { message: res.statusText }, config?.skipToast); } return data as T; }
  private toError(err: any, skipToast = false) { const raw = typeof err?.message === 'string' ? err.message : (typeof err === 'string' ? err : 'Server connection failed'); const message = raw.includes('abort') || raw.includes('signal') ? 'Server response timeout. Please restart dyno or check database/API.' : raw; if (!skipToast) this.toast(message); return { message, error: err }; }
  private toast(message: string, title = 'API Error') { if (!isBrowser) return; const now = Date.now(); if (now - this.lastToast < 8000) return; this.lastToast = now; window.dispatchEvent(new CustomEvent('app-toast', { detail: { title, message, type: 'error', duration: 6000 } })); }
  ...
  async getBlob(url: string, config?: any) { const res = await withTimeout(`${API_URL}${url}`, { headers: this.headers(undefined, config?.headers, url), credentials: 'include' }, 90000); if (!res.ok) throw this.toError(await res.text().catch(() => res.statusText), config?.skipToast); return await res.blob(); }
  async postBlob(url: string, data?: any, config?: any) { const headers = this.headers(data, config?.headers, url); try { await this.ensureCsrfToken(); if (this.csrfToken) headers[this.csrfHeaderName] = this.csrfToken; } catch {} const res = await withTimeout(`${API_URL}${url}`, { method: 'POST', headers, body: JSON.stringify(data || {}), credentials: 'include' }, 90000); if (!res.ok) throw this.toError(await res.text().catch(() => res.statusText), config?.skipToast); return await res.blob(); }
}
...
const getTeachersFromUsers = async () => { const usersData: any = await apiClient.get('/users'); const users = Array.isArray(usersData?.users) ? usersData.users : []; return { teachers: buildTeacherRowsFromUsers(users), fallbackFromUsers: true }; };
...
const teacherApi = { getAll: async (params?: any) => { try { const data: any = await apiClient.get('/teachers', { params }); const teachers = Array.isArray(data?.teachers) ? data.teachers : []; if (teachers.length) return { ...data, teachers }; const fallback = await getTeachersFromUsers(); return { ...data, ...fallback }; } catch { return await getTeachersFromUsers(); } }, create: async (data: any) => { const result = await apiClient.post('/teachers', data); showAppToast('Teacher saved', 'Teacher account/profile saved successfully.', 'success'); return result; }, update: (id: string, data: any) => apiClient.put(`/teachers/${id}`, data), delete: (id: string) => apiClient.delete(`/teachers/${id}`) };
const staffApi = { getAll: (params?: any) => apiClient.get('/staff', { params }), create: (data: any) => apiClient.post('/staff', data), update: (id: string, data: any) => apiClient.put(`/staff/${id}`, data), delete: (id: string) => apiClient.delete(`/staff/${id}`) };
const documentsApi = { getAll: (params?: any) => apiClient.get('/documents', { params }), manage: (params?: any) => apiClient.get('/documents/manage', { params }), upload: (formData: FormData, config?: any) => apiClient.post('/documents/upload', formData, config), delete: (id: string) => apiClient.delete(`/documents/${id}`) };
const holidaysApi = { getAll: (params?: any) => apiClient.get('/holidays', { params }), create: (data: any) => apiClient.post('/holidays', data), update: (id: string) => apiClient.put(`/holidays/${id}`, data), delete: (id: string) => apiClient.delete(`/holidays/${id}`) };
const idCardApi = { getAll: (params?: any) => apiClient.get('/id-cards', { params }), getById: (id: string) => apiClient.get(`/id-cards/${id}`), getMine: (config?: any) => apiClient.get('/id-cards/me/card', config), stats: () => apiClient.get('/id-cards/reports/stats'), searchOwners: (params?: any) => apiClient.get('/id-cards/owners/search', { params }), generate: (data: any) => apiClient.post('/id-cards/generate', data), bulkGenerate: (data: any) => apiClient.post('/id-cards/bulk', data), renew: (id: string, data?: any) => apiClient.post(`/id-cards/${id}/renew`, data), verify: (data: any) => apiClient.post('/id-cards/verify', data), download: (id: string, format: 'pdf' | 'png' = 'pdf') => apiClient.getBlob(`/id-cards/${id}/download?format=${format}`), renderPdf: (data: any) => apiClient.postBlob('/id-cards/render-pdf', data), admitCardPdf: (data: any) => apiClient.postBlob('/id-cards/admit-card/pdf', data), email: (id: string, data: any) => apiClient.post(`/id-cards/${id}/email`, data) };
const backupApi = { getAll: () => apiClient.get('/backup'), create: (data: any) => apiClient.post('/backup', data), export: (collections: string[]) => apiClient.getBlob('/backup/export?collections=' + collections.join(',')), import: (data: any) => apiClient.post('/backup/import', data), restore: (id: string) => apiClient.post(`/backup/${id}/restore`) };
...
export const api: any = {
  ...
  users: { getAll: (params?: any) => apiClient.get('/users', { params }), getAllUsers: () => apiClient.get('/users/all'), updateStatus: (id: string, isActive: boolean) => apiClient.patch(`/users/${id}/status`, { isActive }), updateRole: (id: string, role: string) => apiClient.patch(`/users/${id}/role`, { role }), resetPassword: (id: string, password?: string) => apiClient.post(`/users/${id}/reset-password`, password ? { password } : undefined), permissions: () => apiClient.get('/users/permissions'), updatePermissions: (matrix: any) => apiClient.put('/users/permissions', { matrix }) },
  students: studentApi, teachers: teacherApi, staff: staffApi, documents: documentsApi, notices: crud('/notices'), homework: { getAll: (params?: any) => apiClient.get('/homework', { params }), create: (data: any) => apiClient.post('/homework', data), delete: (id: string) => apiClient.delete(`/homework/${id}`) }, idCards: idCardApi,
  payroll: { previewAttendanceSalary: (params: any) => apiClient.get('/payroll/salary-attendance/preview', { params }), processAttendanceSalary: (data: any) => apiClient.post('/payroll/salary-attendance/process', data) },
  promotions: { preview: (params: any) => apiClient.get('/promotions/preview', { params }), process: (data: any) => apiClient.post('/promotions/process', data), records: (params?: any) => apiClient.get('/promotions/records', { params }) },
  holidays: holidaysApi, backup: backupApi,
  library: { books: crud('/library/books'), loans: { getAll: (params?: any) => apiClient.get('/library/loans', { params }), getById: (id: string) => apiClient.get(`/library/loans/${id}`) }, issue: (d: any) => apiClient.post('/library/loans/issue', d), return: (d: any) => apiClient.post('/library/loans/return') },
  ...
  images: { upload: (formData: FormData) => apiClient.post('/images/upload', formData), delete: (id: string) => apiClient.delete(`/images/${id}`), info: (id: string) => apiClient.get(`/images/${id}/info`) },
  institutionSmsTopup: (d: any) => apiClient.post('/institution/sms/topup', d), institutionSmsTopupPayment: (d: any) => apiClient.post('/institution/sms/topup/payment', d), institutionSmsTopupHistory: (p?: any) => apiClient.get('/institution/sms/topup/history', { params: p }), institutionSmsPackages: () => apiClient.get('/institution/sms/packages'), institutionSmsPurchasePackage: (d: any) => apiClient.post('/institution/sms/package/purchase', d), institutionSmsDiagnostic: () => apiClient.get('/sms-monitoring/sms-diagnostic'), institutionSmsSettings: () => apiClient.get('/sms-monitoring/sms-settings'), institutionSmsSaveSettings: (d: any) => apiClient.post('/sms-monitoring/sms-settings', d), institutionSmsTest: (d: any) => apiClient.post('/sms-monitoring/sms-test', d),
};
