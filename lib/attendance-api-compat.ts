...
const studentRowsFromUsers = (users: any[]) => users
...
const teacherRowsFromUsers = (users: any[]) => users
  ...
  .map((user: any, index: number) => ({
    ...
    userId: { _id: user._id, name: user.name, username: user.username, email: user.email, phone: user.phone, avatar: user.avatar, role: user.role },
  }));

const staffRowsFromUsers = (users: any[]) => users
  .filter((user: any) => ['staff', 'finance_officer', 'librarian', 'accountant'].includes(user?.role))
  .map((user: any, index: number) => ({
    _id: user._id,
    profileId: `user-${user._id}`,
    personType: 'staff',
    employeeId: user.employeeId || `S-${String(index + 1).padStart(3, '0')}`,
    designation: user.designation || (user.role === 'finance_officer' ? 'Finance Officer' : user.role === 'librarian' ? 'Librarian' : 'Staff'),
    department: user.department || '',
    userId: { _id: user._id, name: user.name, username: user.username, email: user.email, phone: user.phone, avatar: user.avatar, role: user.role },
  }));
...
const fallbackPeople = async (personType: string, params?: any) => {
  ...
  if (params?.classId) people = people.filter((student: any) => String(student.classId?._id || student.classId || '') === String(params.classId));
