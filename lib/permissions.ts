...
export const actionPermissionMatrix: Record<PermissionAction, UserRole[]> = {
  'academic:create': ACADEMIC_MANAGE,
  'academic:edit': ACADEMIC_MANAGE,
  'academic:delete': ACADEMIC_MANAGE,
  'academic:publish': [...SCHOOL_LEADERS, 'head'],
  'academic:approve': [...SCHOOL_LEADERS, 'head'],
  'result:create': RESULT_MANAGE,
  'result:edit': RESULT_MANAGE,
  'result:publish': [...SCHOOL_LEADERS, 'head'],
  'result:approve': [...SCHOOL_LEADERS, 'head'],
  'attendance:mark': ['head', 'assistant_head', 'class_teacher', 'subject_teacher', 'teacher'],
  'homework:create': [...SCHOOL_LEADERS, ...TEACHERS, 'admin', 'super_admin'],
  'homework:edit': [...SCHOOL_LEADERS, ...TEACHERS, 'admin', 'super_admin'],
  'homework:delete': [...SCHOOL_LEADERS, ...TEACHERS, 'admin', 'super_admin'],
  'leave:create': LEAVE_APPLY,
  'leave:approve': LEAVE_REVIEW,
  'leave:reject': LEAVE_REVIEW,
  'library:create': LIBRARY_MANAGE,
  'library:edit': LIBRARY_MANAGE,
  'library:delete': LIBRARY_MANAGE,
  'library:issue': LIBRARY_MANAGE,
  'library:return': LIBRARY_MANAGE,
  'sms:view': SMS_MONITORING,
  'student:manage': [...SCHOOL_LEADERS, ...TEACHERS, 'admin', 'super_admin'],
  'settings:manage': ['admin', 'super_admin', 'head'],
};
...
export const routePermissionMatrix: RoutePermission[] = [
  ...
  { path: '/library', roles: LIBRARY_VIEW, match: 'prefix', readOnlyFor: STUDENT_PARENT, scope: 'available-or-own-child-loans' },
...
export function getDashboardQuickActions(role?: UserRole | string): DashboardQuickActionConfig[] {
  const normalizedRole = normalizeUserRole(role);
  if (!normalizedRole) return [];
  return dashboardQuickActionMatrix[normalizedRole] || [
    { label: 'My Profile', href: '/profile', icon: 'UserRound', description: 'View and update your own profile.' },
    { label: 'Notice Board', href: '/notices', icon: 'Bell', description: 'Read published school notices.' },
  ];
}

export function canPerformAction(roleOrUser: UserRole | string | User | null | undefined, action: PermissionAction): boolean {
  if (getDemoMode()) return true;
  const role = typeof roleOrUser === 'string' ? normalizeUserRole(roleOrUser) : normalizeUserRole(roleOrUser?.role);
  if (!role) return false;
  if (['admin', 'super_admin', 'head'].includes(role)) return true;
  return (actionPermissionMatrix[action] || []).includes(role);
}

export function isRoleReadOnlyForRoute(roleOrUser: UserRole | string | User | null | undefined, pathname: string): boolean {
  const role = typeof roleOrUser === 'string' ? normalizeUserRole(roleOrUser) : normalizeUserRole(roleOrUser?.role);
  if (!role) return false;
  const route = findRoutePermission(pathname);
  return Boolean(route?.readOnlyFor?.includes(role));
}
...
export function getVisibleMenuItems(userRole: UserRole | string): MenuItemConfig[] {
...
function findRoutePermission(pathname: string): RoutePermission | undefined {
  ...
  return routePermissionMatrix
    .slice()
    .sort((a, b) => normalizePath(b.path).length - normalizePath(a.path).length)
    .find((route) => {
      const routePath = normalizePath(route.path);
      if ((route.match || 'exact') === 'prefix') return path === routePath || path.startsWith(`${routePath}/`);
